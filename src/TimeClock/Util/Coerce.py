...
class Coercer(object):
    ...
    def __call__(self, *a, **kw):
        ...
        if 'return' in annotations:
            if isinstance(annotations['return'], (list, tuple)):
                rvt = type(annotations['return'])
                new_retval = []
                t = annotations['return'][0]
                for each in retval:
                    if needs_coercing(each, t):
                        each = t(each)
                    if issubclass(t, Interface):
                        verifyObject(t, each)
                    new_retval.append(each)
                retval = rvt(new_retval)
            else:
                if needs_coercing(retval, annotations['return']):
                    retval = annotations['return'](retval)
                if issubclass(annotations['return'], Interface):
                    verifyObject(annotations['return'], retval)
        return retval
    def __repr__(self):
