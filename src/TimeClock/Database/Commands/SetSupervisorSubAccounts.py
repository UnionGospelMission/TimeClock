...
class SetSupervisorSubAccounts(Item):
    name = text(default='Set Supervisor Sub Accounts')
    ...
    @overload
    def execute(self, caller: IAdministrator, employee: ISupervisor, locations: list):
        ...
        for c in current:
            if c not in locations:
                ...
                c.powerDown(employee, ISupervisedBy)
        for a in locations:
            a = ISubAccount(a)
            if a not in current:
                employee.powerUp(a, ISubAccount)
                a.powerUp(employee, ISupervisedBy)
